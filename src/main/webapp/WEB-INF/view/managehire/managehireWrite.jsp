...
<script type="text/javascript">
	$(function() {
		...
		//채용절차 버튼을 눌렀을때 text를 html 화면에 보여줌
	...
	function registerBtnEvent() {
		$("a[name=btn]").click(function(e) {
			...
			switch (btnId) {
			case "btnManagehireSave":
				managehireSave(); // 저장 함수 호출
				break;
			}
		});
	}
	...
	//공고 작성 함수
	function managehireSave() {
	    var getForm = document.getElementById("managehireSaveForm");
	 	// 유효성검사
		if (!validateForm()) {
	    ...
	    const hiringProcText = $("#hiringProcText").text();
	    ...
	    // Ajax 요청
	    callAjaxFileUpload("/manage-hire/managehireSave.do", "post", "json", false, fileData, callback);
	}
	...
	function changeString() {
	    ...
	    let endDate = $("#endDate").val(); // 예시 종료 날짜

	    // startDate와 endDate를 Date 객체로 변환
	    let start = new Date(startDate);
	    let end = new Date(endDate); // end 변수가 여기서 정의됨
	    ...
	    // 포맷팅된 날짜를 다시 저장
	    startDate = startYear+"."+startMonth+"."+startDay+" "+ startHours;
	    ...
	    const checkedValues = $('input[name="expRequired"]:checked')
	        .map(function () {
	    ...
	    const expRequired = checkedValues.join(', ');
	...
	function toggleExpYearsSelect() {
	    // '경력' 체크박스의 선택 여부를 확인
	    const isChecked = $('input[name="expRequired"][value="경력"]').is(':checked');
					...
					<div class="content">
						...
						<p class="Location">
							<a href="/WEB-INF/view/notice/notice.do" class="btn_set home">메인으로</a> <span
								class="btn_nav bold">채용관리</span> <span class="btn_nav bold">공고등록(채용관리자)
						...
						<form id="managehireSaveForm" name="managehireSaveForm" action="" method="post">
                        ...
                        <div id="writeTable">
                            <table class="col" id="jobTable">
							    ...
							    <tr>
							        ...
							        <td>
							             <label><input type="checkbox" name="expRequired" value="신입" class="expCheckbox"> 신입</label>
									    <label><input type="checkbox" name="expRequired" value="경력" class="expCheckbox"> 경력</label>
									    <label><input type="checkbox" name="expRequired" value="경력무관" class="expCheckbox"> 경력무관</label>
									</td>
							        <th>경력</th>
							        <td>
							            <select id="expYears" name="expYears" disabled>
							    ...
							    <tr>
							        ...
							        <td><input type="text" id="openings" name="openings"></td>
							    </tr>
							    <tr>
							        <th>근무지역<span class="font_red">*</span></th>
							        <td><input type="text" id="workLocation" name="workLocation"></td>
							        <th>포지션 설명<span class="font_red">*</span></th>
							        <td><input type="text" id="posDescription" name="posDescription"></td>
							    </tr>
							    <tr>
							        <th>채용기간<span class="font_red">*</span></th>
							        <td colspan="3">
							            시작 <input type="datetime-local" id="startDate" name="startDate">
							    ...
							    <tr>
							        ...
							        <td colspan="3">
							            ...
							            <div id="hiringProcText" name="hiringProcText"></div>
							        </td>
							    </tr>
							    <tr>
							        <th>자격조건<span class="font_red">*</span></th>
							        <td colspan="3"><textarea id="reqQualifications" name="reqQualifications"></textarea></td>
							    </tr>
							    <tr>
							        <th>우대사항</th>
							        <td colspan="3"><textarea id="prefQualifications" name="prefQualifications"></textarea></td>
							    </tr>
							    <tr>
							        <th>업무</th>
							        <td colspan="3"><textarea id="duties" name="duties"></textarea></td>
							    </tr>
							    <tr>
							        <th>혜택 & 복지</th>
							        <td colspan="3"><textarea id="benefits" name="benefits"></textarea></td>
							    </tr>
							    <tr>
							        <th>첨부파일</th>
                            ...
                            <div class="btn_areaC mt30">
                                <a href="" class="btnType blue" id="btnManagehireSave" name="btn"><span>등록</span></a>
                                <a href="" class="btnType gray"><span>돌아가기</span></a>
                            </div>
                        </div>
						</div>
					</form>
