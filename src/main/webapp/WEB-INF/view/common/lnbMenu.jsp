...
<div id="lnb_area">
	<div class="logo">
		<div id="header">
			<a class="logo" href="/dashboard/dashboard.do"> <img
				src="/images/admin/login/logo_img.png" alt="메인페이지" width="80%"
				height="55px" /></a>
		</div>
	</div>
	<div class="login">
		...
		<div class="btn_myPage">
			<a class="mypage" href="/mypage/update.do">MY PAGE</a>
			<!-- /personal/personalInfo.do -->
		</div>
						...
						<c:forEach items="${list.nodeList}" var="i">
							...
							<c:choose>
								<c:when test="${fn:indexOf(urls[fn:length(urls)-1], '=') > -1}">
									<c:set var="url"
										value="${fn:split(urls[fn:length(urls)-1], '=')}" />
									<a href="${i.mnu_url}" id="lnb_${url[fn:length(url)-1]}">-
										${i.mnu_nm}</a>
								</c:when>
								<c:otherwise>
									<c:set var="url"
