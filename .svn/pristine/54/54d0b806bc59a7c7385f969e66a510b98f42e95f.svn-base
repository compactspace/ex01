...
function callAjaxSendUrl(url, method, dataType, async, param, callback) {
    // 기본 메서드는 "post"
    if (method == null || method == "") method = "post";
    ...
    $.ajax({
        ...
        beforeSend: function(xhr) {
            xhr.setRequestHeader("AJAX", "true");
            xhr.setRequestHeader("Request-URL", window.location.href);  // 현재 URL을 헤더에 추가
            $.blockUI({ message: '<h1><img src="/images/admin/comm/busy.gif" /> Just a moment...</h1>', T:99999 });
        },
        success : function(data) {
            callback(data);
...
function selectComCombo(comtype, combo_name, type, code, selvalue){
...
function userCombo(comtype,combo_name, type, code, selvalue){
	  
	  console.log("userCombo Start !!!!!!!!!!!!!! ");
