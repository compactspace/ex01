...
<c:if test="${empty result}">
	<div class="contents">
		<p class="res-comment">보유스킬을 추가할 수 있습니다.</p>
	</div>
</c:if>
...
<c:if test="${not empty result}">
	<div class="contents">
		<table class="col">
			...
			<tr>
				<td><span>${data.skillName}</span></td>
				<td style="white-space: pre-wrap; text-align: left"><span>${data.skillDetail}</span></td>
				<td><button class="deleteBtn" id="skill" data-num="${data.skillIdx}" onclick="deleteSkill(${data.skillIdx})">
